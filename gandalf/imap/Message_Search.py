...
def main(commonConfig,scriptConfig,host):
    ...
    try:
        ...
        imap.select('INBOX')
        logger.info("Search for messages")
        imap.search(None, 'UNSEEN')
        imap.search(None, 'SEEN')
        logger.info("Open first 2 messages")
        imap.fetch('1:2', '(BODY[HEADER.FIELDS (SUBJECT)])')
        logger.info("Search for messages")
        imap.search(None, 'UNSEEN')
        imap.search(None, 'SEEN')
        imap.search(None, "subject", "Test Message 7")
        imap.logout()
    except:
        logger.info("Cleanup")
        mxos.deleteAccount(testAccount)
