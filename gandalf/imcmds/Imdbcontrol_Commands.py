...
def main(commonConfig, scriptConfig, host):
    ...
    imdbcontrolHost = host
    imdbcontrolUsr = scriptConfig['imcmdsUsr']
    imdbcontrolPwd = scriptConfig['imcmdsPwd']
    imdbcontrolDir = scriptConfig['imcmdsDir']
    mailboxId = str(scriptConfig['imcmdsMboxId'])
    mxosHost = commonConfig['hosts']['mxos'].split()[0]
    mxosPort = str(commonConfig['mxosPort'])

    logger.info("------------")
    logger.info("imdbcontrol host: " + imdbcontrolHost)
    logger.info("------------")
    ...
    # Get default domain
    logger.info("- Execute get default domain command")
    cmd = 'source ' + imdbcontrolDir + '.profile;' + imdbcontrolDir + 'bin/imdbcontrol gdd'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(imdbcontrolUsr, imdbcontrolHost, cmd, imdbcontrolPwd)
        logger.info( output )
    except Exception as e:
        mxos.deleteAccount(testAccount)
        exit(1)

    # List class of services
    logger.info("- Execute list class of services command")
    cmd = 'source ' + imdbcontrolDir + '.profile;' + imdbcontrolDir + 'bin/imdbcontrol lcn'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(imdbcontrolUsr, imdbcontrolHost, cmd, imdbcontrolPwd)
        logger.info( output )
    ...
    cmd = 'source ' + imdbcontrolDir + '.profile;' + imdbcontrolDir + 'bin/imdbcontrol ca ' + testAccount.username + ' ' + testAccount.messageStoreHost + ' ' + mailboxId + ' ' + testAccount.username + ' ' + testAccount.password + ' ' + testAccount.passwordStoreType + ' ' + testAccount.domain + ' A S ' + testAccount.cos
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(imdbcontrolUsr, imdbcontrolHost, cmd, imdbcontrolPwd)
        logger.info( output )
    ...
    # Get account
    logger.info("- Execute get account command")
    cmd = 'source ' + imdbcontrolDir + '.profile;' + imdbcontrolDir + 'bin/imdbcontrol ga ' + testAccount.username + ' ' + testAccount.domain
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(imdbcontrolUsr, imdbcontrolHost, cmd, imdbcontrolPwd)
        logger.info( output )
    ...
    # Modify account
    logger.info("- Execute modify pop address command")
    cmd = 'source ' + imdbcontrolDir + '.profile;' + imdbcontrolDir + 'bin/imdbcontrol map ' + testAccount.username + ' ' + testAccount.domain + ' ' + testAccount.username + 'modified'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(imdbcontrolUsr, imdbcontrolHost, cmd, imdbcontrolPwd)
        logger.info( output )
    ...
    cmd = 'source ' + imdbcontrolDir + '.profile;' + imdbcontrolDir + 'bin/imdbcontrol da ' + testAccount.username + ' ' + testAccount.domain
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(imdbcontrolUsr, imdbcontrolHost, cmd, imdbcontrolPwd)
        logger.info( output )
