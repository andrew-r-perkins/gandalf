...
def description():
    ...
    #############################################################
    print "Update the account's status via MxOS"
...
def main(commonConfig, scriptConfig, host):
    ...
    # Get account status before the account is created.
    mxos.getAccountStatus(testAccount)

    # Create the account in Mx
    mxos.createAccount(testAccount)
    automated_testing_common.sleep(commonConfig['sleepIntervalSec'], logger)
    mxos.getAccountStatus(testAccount)

    # Update account status
    mxos.updateAccountStatus(testAccount, 'proxy')
    automated_testing_common.sleep(commonConfig['sleepIntervalSec'], logger)
    mxos.getAccountStatus(testAccount)
