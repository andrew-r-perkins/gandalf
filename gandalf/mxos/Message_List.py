...
def description():
    ...
    #############################################################
    print "Get messages in a folder via MxOS"
...
def main(commonConfig,scriptConfig,host):
    ...
    INBOX = str(scriptConfig['INBOX'])
    Trash = str(scriptConfig['Trash'])
    ...
    smtp = automated_testing_common.Smtp(logger=logger, host=smtpHost, port=smtpPort, useSsl=False)
    ...
    msg = automated_testing_common.Message(From=testAccount.getFullQualifiedName(), To=testAccount.getFullQualifiedName(),
                                           subject='Test Message', bodyText='This is an automated test message.')

    try:

        # Send 5 messages using the template message

        # 30/04/2024 - Steve Thomson
        #              Added testAccount to parameter list.  Without this
        #              parameter the function would fail with the follwoing error:
        #              sendMultipleMessage() takes exactly 4 arguments (3 given)

        #smtp.sendMultipleMessage(msg,5)
        smtp.sendMultipleMessage(msg,testAccount,5)

        # Sleep to make sure sends have been processed.
        automated_testing_common.sleep(commonConfig['sleepIntervalSec'], logger)

        # Display messages from INBOX and Trash folders.
        logger.info("List messages in INBOX and " + Trash + " folders")
        mxos.messageList(testAccount, INBOX)
        mxos.messageList(testAccount, Trash)
