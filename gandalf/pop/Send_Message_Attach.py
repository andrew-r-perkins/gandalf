...
def description():
    ...
    #############################################################
    print "Send a message with attachment"
...
def main(commonConfig,scriptConfig,host):
    ...
    popHost = host
    popSSL = str(commonConfig['popSSL'])
    if popSSL == "YES" :
        popPort = str(commonConfig['popPortSSL'])
        popUseSsl = True
    else:
        popPort = str(commonConfig['popPort'])
        popUseSsl = False
    ...
    logger.info("------------")
    logger.info("POP server:   " + popHost)
    logger.info("POP port:     " + popPort)
    logger.info("POP use SSL:  " + str(popUseSsl))
    ...
    logger.info("Create connections to servers")
    pop = automated_testing_common.Pop(logger=logger, host=popHost, port=popPort, useSsl=popUseSsl)
    ...
    logger.info("Create test account")
    ...
    msg.addAttachment('./pop/attachment.jpg')
    ...
    try:
        ...
        smtp.sendMessage(msg, testAccount)
        automated_testing_common.sleep(commonConfig['sleepIntervalSec'], logger)
        pop.login(testAccount)
        pop.getStat()
        pop.getList()
        pop.quit()
    except:
