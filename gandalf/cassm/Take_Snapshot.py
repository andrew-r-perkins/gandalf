...
def main(commonConfig, scriptConfig, host):
    ...
    cassmHost = host
    cassmPort = commonConfig['cassmPort']
    cassmKS = commonConfig['cassmKS']
    cassmUsr = scriptConfig['cassmUsr']
    cassmPwd = scriptConfig['cassmPwd']
    cassmDir = scriptConfig['cassmDir']

    logger.info("------------")
    logger.info("Cassandra host:     " + cassmHost)
    logger.info("Cassandra port:     " + str(cassmPort))
    logger.info("Cassandra keyspace: " + cassmKS)
    logger.info("------------")

    # Execute nodetool snapshot
    logger.info("Execute nodetool snapshot")
    cmd = cassmDir + "nodetool snapshot -t testsnapshot001"
    try:
        output = automated_testing_common.runSshCmd(cassmUsr, cassmHost, cmd, cassmPwd)
        logger.info( output )
    except Exception as e:
        exit(1)

    # Execute nodetool listsnapshot
    logger.info("Execute nodetool listsnapshots")
    cmd = cassmDir + "nodetool listsnapshots"
    try:
        output = automated_testing_common.runSshCmd(cassmUsr, cassmHost, cmd, cassmPwd)
        logger.info( output )
    except Exception as e:
    ...
    # Execute nodetool clearsnapshot
    logger.info("Execute nodetool clearsnapshot")
    cmd = cassmDir + "nodetool clearsnapshot -t testsnapshot001"
    try:
        output = automated_testing_common.runSshCmd(cassmUsr, cassmHost, cmd, cassmPwd)
        logger.info( output )
    except Exception as e:
