...
def description():
    ...
    #############################################################
    print "Check using 'imlogsum -l 3 <mxserv.log>'."
...
def main(commonConfig, scriptConfig, host):
    ...
    try:
        ...
        hostnameFilePath = '~imail/config/hostname'
        cmd = 'source ~imail/.profile; cat ' + hostnameFilePath
        ...
        if 'No such file or directory' in cmdOutput:
            raise ValueError('Mx hostname file was not found at ' + host + ':' + hostnameFilePath)

        mxHostname = cmdOutputLines[0]
        logger.debug( mxHostname )

        # Grep config.db for Mx servers running on host.

        configDbFilePath = '~imail/config/config.db'
        cmd = "grep _run: " + configDbFilePath + " | grep '\[on\]' | grep " + mxHostname + " | sed -e s/_run:.*// | sed -e s:.*/::"
        logger.debug( cmd )
        ...
        mxServerList = cmdOutputLines
        logger.debug( mxServerList )

        # Imlogsum 
        for mxServer in mxServerList:

            # If empty list entry, move on to the next.
            if mxServer is '':
                continue

            logFilePath = '~imail/log/' + mxServer + '.log'
            cmd = 'source ~imail/.profile; imlogsum -l 3 ' + logFilePath
            logger.debug( cmd )
            ...
            if 'No such file or directory' in cmdOutput:
                raise ValueError('Mx log file was not found at ' + host + ':' + logFilePath)

            insideBraces = False
            foundLogSumEnter = False
            logSumEntriesList = []
            ...
            for line in cmdOutputLines:
                if not insideBraces:
                    ...
                    if '}' in line:
                        ...
                        if line:
                            foundLogSumEnter = True
                            checkPassed = False
                            numLogEvents, logEvent = line.strip().split(':')
                            logSumEntriesList.append( strFormat.format( '  ' + logEvent, numLogEvents, '' ) )

            logSumStr = 'imlogsum ' + mxServer 
            if foundLogSumEnter:
                logger.warning( strFormat.format( logSumStr, '', '***IMLOGSUM ALARM***' ) )
                for line in logSumEntriesList:
                    logger.warning( line )
            else:
                logger.info( logSumStr )
