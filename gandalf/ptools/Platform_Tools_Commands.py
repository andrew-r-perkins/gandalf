...
def main(commonConfig, scriptConfig, host):
    ...
    ptoolsHost = host
    ptoolsUsr = scriptConfig['ptoolsUsr']
    ptoolsPwd = scriptConfig['ptoolsPwd']
    ptoolsDir = scriptConfig['ptoolsDir']
    profilePath = scriptConfig['imailProfilePath']
    
    if str(commonConfig['ptoolsEncrypt']) == "YES":
        addEncryptSwitch = True
    else:
        addEncryptSwitch = False
    ...
    logger.info("------------")
    logger.info("Platform tools host: " + ptoolsHost)
    logger.info("Encrypt switch     : " + str(addEncryptSwitch))
    ...
    # Execute account-create command.
    logger.info("- Execute account-create command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'account-create ' + testAccount.getFullQualifiedName() + ' ' + testAccount.password + ' ' + testAccount.cosId
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
        logger.info( output )
    except Exception as e:
        mxos.deleteAccount(testAccount)
        exit(1)

    # Execute imboxstats command.
    logger.info("- Execute imboxstats command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'imboxstats ' + testAccount.getFullQualifiedName()
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
    ...
    # Execute account-snapshot command.
    logger.info("- Execute account-snapshot command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'account-snapshot ' + testAccount.getFullQualifiedName() 
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
    ...
    # Execute imfolderlist command.
    logger.info("- Execute imfolderlist command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'imfolderlist ' + testAccount.getFullQualifiedName() 
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
    ...
    # Execute immsaccesscontrol command.
    logger.info("- Execute immsaccesscontrol command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'immsaccesscontrol ' + testAccount.getFullQualifiedName() + " show"
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
    ...
    # Execute immssboxdelete command.
    logger.info("- Execute immssboxdelete command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'immssboxdelete ' + testAccount.getFullQualifiedName()
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    ...
    # Execute immssboxcreate command.
    logger.info("- Execute immssboxcreate command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'immssboxcreate ' + testAccount.getFullQualifiedName()
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
    ...
    # Execute account-delete command.
    logger.info("- Execute account-delete command")
    cmd = 'source ' + profilePath + ';' + ptoolsDir + 'account-delete ' + testAccount.getFullQualifiedName()
    if addEncryptSwitch: cmd = cmd + ' -sslEnabled true'
    logger.info(cmd)
    try:
        output = automated_testing_common.runSshCmd(ptoolsUsr, ptoolsHost, cmd, ptoolsPwd)
