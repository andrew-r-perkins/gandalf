...
def description():
    ...
    #############################################################
    print "Send large messages via MTA"
...
def main(commonConfig,scriptConfig,host):
    ...
    attachmentLocation = scriptConfig['attachmentLocation']
    INBOX = "/" + str(scriptConfig['INBOX'])
    SentMail = "/" + str(scriptConfig['SentMail'])
    Drafts = "/" + str(scriptConfig['Drafts'])
    Spam = "/" + str(scriptConfig['Spam'])
    ...
    logger.info("Attachment Location:   " + attachmentLocation)
    logger.info("------------")
    ...
    try:
        ...
        msg.addAttachment(attachmentLocation)
    ...
    try:
        ...
        # Verify 5 messages in INBOX
        foldersToCheck = { INBOX    : 5,
                           Drafts   : 0 }
        mxos.verifyNumMsgsInFolders(testAccount,foldersToCheck)
    except:
        logger.info("Cleanup")
        mxos.deleteAccount(testAccount)
        exit(1)
